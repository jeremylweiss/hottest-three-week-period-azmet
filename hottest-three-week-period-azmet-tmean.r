...
# This code calculates and graphs a 21-day moving average of daily mean 
...
filter_length <- 21
stn_data["Tmean_mavg"] <- NA
stn_data$Tmean_mavg <- stats::filter(
  x = stn_data$Tmean, 
...
# Calculate average mean temperature by Julian day and moving average of 
# these daily average values for the station period of record
stn_data["Tmean_JDay_avg"] <- NA
for (jd in min(stn_data$JDay):max(stn_data$JDay)) {
  df <- filter(stn_data, JDay == jd)
  stn_data$Tmean_JDay_avg[which(stn_data$JDay == jd)] <- mean(df$Tmean)
}
rm(jd, df)

stn_data["Tmean_JDay_mavg"] <- NA
for (yr in min(stn_data$Year):max(stn_data$Year)) {
  df <- filter(stn_data, Year == yr)
  df$Tmean_JDay_mavg <- stats::filter(
    x = df$Tmean_JDay_avg, 
    filter = rep(1, filter_length) / filter_length,
    method = "convolution",
    sides = 2,
  ...
  stn_data$Tmean_JDay_mavg[which(stn_data$Year == yr)] <- df$Tmean_JDay_mavg
}
rm(yr, df)
...
for (yr in min(stn_data_plot$Year):max(stn_data_plot$Year)) {
  ...
  print(yr)
  print(head(sort(df$Tmean_mavg, decreasing = TRUE)))
  print(head(sort(df$Tmean_JDay_mavg, decreasing = TRUE)))
  flush.console()
}
rm(yr, df)
...
for (yr in min(stn_data_plot$Year):max(stn_data_plot$Year)) {
  ...
  mavg_window_yr$Start[which(mavg_window_yr$Year == yr)] <-
    df$JDay[which(df$Tmean_mavg == max(df$Tmean_mavg))] - 10
  ...
  mavg_window_yr$Month_start[which(mavg_window_yr$Year == yr)] <- 
    stn_data_plot$Month[
      which(stn_data_plot$Year == yr &
              stn_data_plot$Tmean_mavg == max(df$Tmean_mavg)) - 10
      ]
...
mavg_window_avg <- data.frame(
  Month_start = df$Month[
    which(df$Tmean_JDay_mavg == max(df$Tmean_JDay_mavg, na.rm = TRUE))[1] - 10
    ],
  ...
  Start = df$JDay[
    which(df$Tmean_JDay_mavg == max(df$Tmean_JDay_mavg, na.rm = TRUE))[1]
    ] - 10,
  End = df$JDay[
    which(df$Tmean_JDay_mavg == max(df$Tmean_JDay_mavg, na.rm = TRUE))[1]
    ] + 10
...
analysis_window <- 
  data.frame(x_min = 135, x_max = 196, y_min = min(stn_data_plot$Tmean), 
             y_max = max(stn_data_plot$Tmean))
...
p <- ggplot(data = stn_data_plot) +
  ...
  geom_segment(
    mapping = aes(x = (135 + 1), xend = (196 - 1), y = 52.5, yend = 52.5),
    arrow = arrow(angle = 45, length = unit(1.5, "mm"), ends = "both", 
                  type = "open"),
    color = "gray70", size = 0.35
  ) +
  
  geom_text(mapping = aes(x = ((135 + 196) / 2), y = (52.5 + 1)),
            label = "period of interest", family = "Source Sans Pro", 
            fontface = "plain", hjust = "center", vjust = "bottom", 
            size = 7 / .pt, color = "gray70", angle = 0) +
  
  # Add Tmean 21-day moving average
  geom_line(mapping = aes(x = JDay, y = Tmean_mavg, color = factor(Year))) +
  
  # Add Tmean daily values
  geom_point(mapping = aes(x = JDay, y = Tmean, color = factor(Year)), 
             alpha = 0.5) +
  
  # Add moving average windows for individual years
  geom_segment(data = mavg_window_yr,
               mapping = aes(x = Start, xend = End, y = 67.5, yend = 67.5),
               color = viridis(n = 4, alpha = 1.0, begin = 0.0, end = 0.75, 
                               direction = -1),
               size = 3) +
  
  geom_text(data = mavg_window_yr, mapping = aes(x = (Start - 1), y = 67.5),
  ...
  geom_text(data = mavg_window_yr, mapping = aes(x = (End + 1), y = 67.5),
  ...
  annotate(geom = "segment", 
           ...
           y = 62.5, yend = 62.5, alpha = 1.0, color = "gray50", size = 3) +
  
  annotate(geom = "text", x = (mavg_window_avg$Start - 1), y = 62.5, 
  ...
  annotate(geom = "text", x = (mavg_window_avg$End + 1), y = 62.5, 
  ...
  scale_y_continuous(
    breaks = seq(from = 0, to = (max(stn_data_plot$Tmean, na.rm = TRUE) + 1), 
                 by = 10),
    limits = c(
      (min(filter(stn_data, JDay >= 119 & JDay <= 215)$Tmean, na.rm = TRUE) - 4),
      (max(filter(stn_data, JDay >= 119 & JDay <= 215)$Tmean, na.rm = TRUE) + 2)),
    expand = c(0.0, 0.0)
  ) +
  
  # Add the title, subtitle, axis labels, and caption
  ggtitle("Warmest 21-Day Period of Mean Temperature") +
...
ggsave(file = paste0("hottest-three-week-period-azmet-tmean-willcox-bench-",
                     Sys.Date(),
                     ".eps"),
