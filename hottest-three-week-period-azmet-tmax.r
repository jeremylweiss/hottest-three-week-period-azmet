...
p <- ggplot(data = stn_data_plot) +
  ...
  # Add 95F threshold line
  geom_hline(yintercept = 95, color = "gray40", linetype = "dashed", 
             size = 0.5) +
  
  geom_text(mapping = aes(x = (119 + 2), y = (95 + 1)),
            label = "95°F", family = "Source Sans Pro", 
            fontface = "plain", hjust = "left", vjust = "bottom", 
            size = 8 / .pt, color = "gray40", angle = 0) +
...
ggsave(file = paste0("hottest-three-week-period-azmet-tmax-willcox-bench-",
                     Sys.Date(),
                     ".eps"),
